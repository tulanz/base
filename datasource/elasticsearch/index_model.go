...
func NewIndexModel(client *elastic.Client, logger *zap.Logger, model interface{}, tenantId string) *IndexModel {
	return &IndexModel{client, logger, model, tenantId}
}
...
func (m *IndexModel) CreateIndex(c context.Context) error {
	...
	if !indexExists {
		...
		m.logger.Info("Index", zap.String("indexName", indexName), zap.Any("mapping", mapping))
		r, err := m.client.CreateIndex(indexName).BodyJson(mapping).Do(c)
		if err != nil {
			m.logger.Error("CreateIndex Error", zap.Error(err))
			return err
		}
